...
const MenuPage = () => {
  ...
  const { cartItems, addToCart, clearCart, totalItemsCount, subtotal, customerName } = useCart();
  ...
  const filteredFoods = foods.filter((food) => {
    const matchesCategory = selectedCategory ? food.category?._id === selectedCategory : true;
    const nameEn = (food.name?.en || '').toLowerCase();
    const nameAm = (food.name?.am || '').toLowerCase();
    const query = searchQuery.toLowerCase().trim();
    const matchesSearch = !query || nameEn.includes(query) || nameAm.includes(query);
  ...
  if (tableError) {
    return (
      <div className="min-h-screen bg-cafe-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-bold text-cafe-900 mb-2">{t('invalid_table_title')}</h2>
